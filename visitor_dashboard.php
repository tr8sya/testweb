...
$showActionButtons = false; // Variable to control visibility of action buttons
...
if (isset($_POST['register'])) {
    ...
    } elseif (!preg_match('/^01\d{9,11}$/', $visitorPhone)) {
        $error .= "<br>Phone number must start with '01' and be between 10 to 12 digits long.";
    } else {
    ...
    if (empty($error)) {
        ...
        if ($checkPatient->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                $showActionButtons = true; // Show the action buttons after registration
            } else {
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" onsubmit="return validateForm()">
            ...
            <?php if ($showActionButtons): ?>
                <div class="action-buttons">
                    <a href="update_visitor.php" class="button">Update</a>
                    <a href="exit_visitor.php" class="button">Exit</a>
                </div>
            <?php endif; ?>
